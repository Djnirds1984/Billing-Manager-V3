...
const apiCall = async <T>(router: RouterConfigWithId, endpoint: string, method: string = 'GET', body?: any): Promise<T> => {
    // A special 'test' ID is used for connection testing before a router is saved.
    const routerId = router.id === 'test' ? 'test' : router.id;
    const url = `${BASE_URL}/${routerId}/${endpoint.replace(/^\//, '')}`;
...
export const getInterfaceStats = (router: RouterConfigWithId) => apiCall<Interface[]>(router, 'interface/stats');

// --- DHCP Clients (Portal) ---
export const getDhcpClients = (router: RouterConfigWithId) => apiCall<DhcpClient[]>(router, 'ip/dhcp-server/lease/print');
export const updateDhcpClientDetails = (router: RouterConfigWithId, client: DhcpClient, params: DhcpClientActionParams) => apiCall(router, 'dhcp-client/update', 'POST', { macAddress: client.macAddress, address: client.address, ...params });
export const deleteDhcpClient = (router: RouterConfigWithId, client: DhcpClient) => apiCall(router, 'ip/dhcp-server/lease/remove', 'POST', { '.id': client.id });
export const runDhcpCaptivePortalSetup = (router: RouterConfigWithId, params: DhcpCaptivePortalSetupParams) => apiCall<{message: string}>(router, 'script/run-dhcp-portal-setup', 'POST', params);
export const runDhcpCaptivePortalUninstall = (router: RouterConfigWithId) => apiCall<{message: string}>(router, 'script/run-dhcp-portal-uninstall', 'POST');
...
export const processPppPayment = (router: RouterConfigWithId, paymentData: any) => apiCall(router, 'ppp/payment/process', 'POST', paymentData); // Custom backend endpoint
export const savePppUser = (router: RouterConfigWithId, data: any) => apiCall(router, 'ppp/user/save', 'POST', data); // Custom backend endpoint
...
export const runHotspotSetup = (router: RouterConfigWithId, params: HotspotSetupParams) => apiCall<{message: string}>(router, 'script/run-hotspot-setup', 'POST', params);
...
export const getWanRoutes = (router: RouterConfigWithId) => apiCall<WanRoute[]>(router, 'ip/route/print');
export const getWanFailoverStatus = (router: RouterConfigWithId) => apiCall<FailoverStatus>(router, 'system/script/wan-failover-status');
